...
assert_equal(marketplace.get("name"), "sjujperpowers-dev", "marketplace name")
assert_equal(
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "sjujperpowers"]
assert_equal(len(matching_plugins), 1, "sjujperpowers plugin entry count")
