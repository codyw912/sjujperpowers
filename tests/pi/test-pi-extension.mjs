...
const extensionPath = resolve(repoRoot, '.pi/extensions/sjujperpowers.ts');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'sjujperpowers');
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have sjujperpowers/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have sjujperpowers/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have sjujperpowers/);
