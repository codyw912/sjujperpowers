...
PLAN
    ...
    if [[ "$dir_a" == "$repo/.sjujperpowers/sdd/plan-a" ]]; then
        pass "prints <repo-root>/.sjujperpowers/sdd/<plan-basename>"
    else
        fail "prints <repo-root>/.sjujperpowers/sdd/<plan-basename>"
        echo "    got: $dir_a"
    fi
    ...
    if [[ -f "$repo/.sjujperpowers/sdd/.gitignore" && "$(cat "$repo/.sjujperpowers/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created at .sjujperpowers/sdd/ with '*'"
    else
        fail "self-ignoring .gitignore created at .sjujperpowers/sdd/ with '*'"
    fi
    ...
    if [[ "$brief_path" == "$repo/.sjujperpowers/sdd/plan-a/task-1-brief.md" ]]; then
