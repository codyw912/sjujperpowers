...
EOF
    ...
    cat > "$repo/assets/sjujperpowers-small.svg" <<'EOF'
...
EOF
    ...
    git -C "$repo" add \
        ...
        assets/app-icon.png \
        assets/sjujperpowers-small.svg \
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/sjujperpowers/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/sjujperpowers/.fixture-keep"
    cat > "$repo/plugins/sjujperpowers/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/sjujperpowers/.fixture-keep
    git -C "$repo" add plugins/sjujperpowers/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/sjujperpowers/skills/example/agents"

    cat > "$repo/plugins/sjujperpowers/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/sjujperpowers/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/sjujperpowers/.codex-plugin" \
        "$repo/plugins/sjujperpowers/.private-journal" \
        "$repo/plugins/sjujperpowers/assets" \
        "$repo/plugins/sjujperpowers/hooks" \
        "$repo/plugins/sjujperpowers/skills/example/agents" \
        "$repo/plugins/sjujperpowers/skills/example"

    cat > "$repo/plugins/sjujperpowers/.codex-plugin/plugin.json" <<EOF
{
  "name": "sjujperpowers",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/sjujperpowers/assets/sjujperpowers-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/sjujperpowers/assets/app-icon.png"

    cat > "$repo/plugins/sjujperpowers/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/sjujperpowers/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/sjujperpowers/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/sjujperpowers/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/sjujperpowers/hooks/session-start" "$repo/plugins/sjujperpowers/hooks/session-start-codex" "$repo/plugins/sjujperpowers/hooks/run-hook.cmd"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/sjujperpowers/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/sjujperpowers/.codex-plugin/plugin.json \
        plugins/sjujperpowers/assets/app-icon.png \
        plugins/sjujperpowers/assets/sjujperpowers-small.svg \
        plugins/sjujperpowers/hooks/hooks-codex.json \
        plugins/sjujperpowers/hooks/run-hook.cmd \
        plugins/sjujperpowers/hooks/session-start \
        plugins/sjujperpowers/hooks/session-start-codex \
        plugins/sjujperpowers/skills/example/agents/openai.yaml \
        plugins/sjujperpowers/skills/example/SKILL.md \
        plugins/sjujperpowers/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/sjujperpowers/.kimi-plugin" \
        "$repo/plugins/sjujperpowers/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/sjujperpowers/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/sjujperpowers/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/sjujperpowers/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/sjujperpowers/.fixture-keep \
        plugins/sjujperpowers/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/sjujperpowers/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/sjujperpowers/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/sjujperpowers-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (sjujperpowers-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/sjujperpowers-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/sjujperpowers/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/sjujperpowers" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/sjujperpowers-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/sjujperpowers'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/sjujperpowers-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/sjujperpowers-*" "Clean no-op local apply does not create sync branch in destination checkout"
