...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SJUJPERPOWERS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$SJUJPERPOWERS_SKILLS_DIR/using-sjujperpowers/SKILL.md" "$TEST_HOME/using-sjujperpowers.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SJUJPERPOWERS_PLUGIN_FILE" missing
}
