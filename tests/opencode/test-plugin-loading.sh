#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the sjujperpowers plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/sjujperpowers.js"
...
skill_count=$(find "$SJUJPERPOWERS_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $SJUJPERPOWERS_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-sjujperpowers skill exists (critical for bootstrap)
echo "Test 3: Checking using-sjujperpowers skill (required for bootstrap)..."
if [ -f "$SJUJPERPOWERS_SKILLS_DIR/using-sjujperpowers/SKILL.md" ]; then
    echo "  [PASS] using-sjujperpowers skill exists"
else
    echo "  [FAIL] using-sjujperpowers skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SJUJPERPOWERS_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/sjujperpowers/' "$SJUJPERPOWERS_PLUGIN_FILE"; then
