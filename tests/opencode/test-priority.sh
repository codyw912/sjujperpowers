...
# 1. Create in sjujperpowers location (lowest priority)
mkdir -p "$SJUJPERPOWERS_SKILLS_DIR/priority-test"
cat > "$SJUJPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Sjujperpowers version of priority test skill
---
# Priority Test Skill (Sjujperpowers Version)

This is the SJUJPERPOWERS version of the priority test skill.

PRIORITY_MARKER_SJUJPERPOWERS_VERSION
EOF
...
if [ -f "$SJUJPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Sjujperpowers version exists"
else
    echo "  [FAIL] Sjujperpowers version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled sjujperpowers priority
echo ""
echo "Test 2: Documenting personal vs sjujperpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled sjujperpowers version instead of personal version"

# Test 3: Document project vs bundled sjujperpowers priority
echo ""
echo "Test 3: Documenting project vs personal/sjujperpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled sjujperpowers version instead of project version"

# Test 4: Test a non-colliding bundled sjujperpowers skill is still available
echo ""
echo "Test 4: Testing non-colliding sjujperpowers skill remains available..."

mkdir -p "$SJUJPERPOWERS_SKILLS_DIR/sjujperpowers-only-test"
cat > "$SJUJPERPOWERS_SKILLS_DIR/sjujperpowers-only-test/SKILL.md" <<'EOF'
---
name: sjujperpowers-only-test
description: Sjujperpowers-only priority test skill
---
# Sjujperpowers Only Test Skill

PRIORITY_MARKER_SJUJPERPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"sjujperpowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SJUJPERPOWERS_ONLY_VERSION" "Non-colliding sjujperpowers skill is still registered"
